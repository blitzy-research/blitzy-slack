...
| 4 | [frame 452](../../screenshots/Slack%20web%20Jul%202024%20452.png), [frame 453](../../screenshots/Slack%20web%20Jul%202024%20453.png) | Types a new name, then sets the list's emoji | The input holds the new name with cancel and save still available and the modal title still showing the old name. The emoji control at the field's left **changes between the two captures**: a neutral outlined placeholder glyph at [frame 452](../../screenshots/Slack%20web%20Jul%202024%20452.png), a chosen gold star emoji at [frame 453](../../screenshots/Slack%20web%20Jul%202024%20453.png), with the name text identical in both | `C-MODAL-SHELL` |
...
> **Build obligation:** **the export needs an authorization, a scope and a formula defence.** The download-CSV affordance appears both in the list overflow menu and in the details modal [frame 449](../../screenshots/Slack%20web%20Jul%202024%20449.png), [frame 450](../../screenshots/Slack%20web%20Jul%202024%20450.png). Per `S-EXPORT`: it is authorized as its own operation; it is **scoped to the exporter's authorized set** rather than to the whole list where field-level audiences differ; it is audited and rate-limited; and **every exported cell is put through the single force-text neutralisation `S-EXPORT` fixes** — canonicalise the value first, stripping leading whitespace, control, zero-width and bidirectional-override characters, *then* force it to text with a leading apostrophe if what remains begins with an equals, plus, minus, at, tab, carriage return or line feed. **Field quoting is not an alternative**: a spreadsheet strips a quoted field's quotes and evaluates what was inside, so `"=..."` executes exactly as `=...` does, and canonicalising first is what stops a single leading space from carrying the formula past the test. Without this, a record's title becomes an expression that executes in the reader's application. Person, email, phone and date fields are additionally **minimised**, per the field-audience model owned by [15-admin-workspace.md](15-admin-workspace.md).
...
> **Build obligation:** **the feedback control's payload is specified or it transmits nothing.** The overflow menu carries a share-feedback row whose destination and payload are not captured [frame 449](../../screenshots/Slack%20web%20Jul%202024%20449.png). Per `S-TELEMETRY` — the payload contract, **not** the device-capture one — the build either states what is transmitted, assembling it from a declared field allowlist that **excludes list content, record values, person fields and identifiers**, or transmits nothing beyond a free-text message the person composed; the destination is bounded to a named first-party endpoint or a support channel the person chose rather than an automatic upload; and per `S-RETENTION` the stored payload carries a stated retention bound and a deletion path either way.
...
- [ ] `E-LIST` and `E-LIST-RECORD` persist every **shared** field listed for them in the **Implied data model** section of this document — the fields whose value is the same for every viewer, which the corpus confirms for saved views, since the save-a-view modal's own note states that views can be seen by everyone with access to the list [frame 485](../../screenshots/Slack%20web%20Jul%202024%20485.png) — while the starred flag, the last-viewed time, an **unsaved** view configuration and notification subscription live on the viewer–list relation required by the `S-PERUSER` criterion below and never on either entity. The shared set includes the list's emoji, its creator and last editor with times, its ordered field set with per-field type and per-type options, and its **named, saved** views with their layout, filters, sorts, hidden fields and grouping field — each field carrying its own citation there, of which the emoji [frame 451](../../screenshots/Slack%20web%20Jul%202024%20451.png), the creator and last editor with their times [frame 449](../../screenshots/Slack%20web%20Jul%202024%20449.png), the named and counted views [frame 464](../../screenshots/Slack%20web%20Jul%202024%20464.png), the layout selector [frame 469](../../screenshots/Slack%20web%20Jul%202024%20469.png), the hidden-field toggles [frame 479](../../screenshots/Slack%20web%20Jul%202024%20479.png) and the group-by chip [frame 483](../../screenshots/Slack%20web%20Jul%202024%20483.png) are the densest sources.
...
- [ ] The CSV export is authorized as its own operation, **scoped to the exporter's authorized set**, audited, rate-limited, and every exported cell passes through the **single tested force-text neutralisation** `S-EXPORT` fixes — canonicalised first, then prefixed with a leading apostrophe if it begins with an equals, plus, minus, at, tab, carriage return or line feed, with quoting handled separately and **never relied on as the defence**. The build asserts on a produced file that such a value, and the same value behind a leading space or zero-width character, is read as text [frame 449](../../screenshots/Slack%20web%20Jul%202024%20449.png), [frame 450](../../screenshots/Slack%20web%20Jul%202024%20450.png).
...
- [ ] The share-feedback control either states exactly what it transmits — assembled from a declared field allowlist excluding list content, record values, person fields and identifiers — or transmits nothing beyond a message the person composed; its destination is bounded rather than an automatic upload; and the stored payload carries a stated retention bound and a deletion path either way, per `S-TELEMETRY` and `S-RETENTION` [frame 449](../../screenshots/Slack%20web%20Jul%202024%20449.png).
