...
flowchart TD
    ...
    EXTERNAL["External address - disambiguate, set channel permissions, send invitation"]
    OPEN["Channel open in the content region"]
    DETAILS["Channel details modal"]
    ABOUT["About tab - name, topic, description, creator, files, identifier"]
    ...
    NOTIFY["Notification preference - all messages, mentions, off or muted"]
    ...
    ARCHIVED["Archived channel - read only, composer replaced by a status bar"]
...
An authenticated session with a workspace loaded. No existing channel, membership or entitlement is required to reach step 1: the wizard opens with an empty name field and its forward action already rendered muted [frame 58](../../screenshots/Slack%20web%20Jul%202024%2058.png).
...
| 3 | [frame 60](../../screenshots/Slack%20web%20Jul%202024%2060.png) | Advances to step 2 | The modal's body is replaced: a sub-line renders the channel name prefixed by a hash glyph, a Visibility label heads a radio group whose first option is public with a sub-label scoping it to everyone in the workspace and whose second is private with a sub-label stating it can only be viewed or joined by invitation. Public is pre-selected. The footer now reads step 2 of 2 and offers a back action then a create action | `C-MODAL-SHELL`, `C-STEP-WIZARD` |
...
An authenticated session in a workspace holding an archived channel that the user may administer. Reading the channel requires no membership — the header carries no member count at all — but unarchiving is reached only through the details modal [frame 134](../../screenshots/Slack%20web%20Jul%202024%20134.png), [frame 136](../../screenshots/Slack%20web%20Jul%202024%20136.png).
...
A first authenticated session for this account in this workspace, with browser notification permission neither granted nor denied — the request band is only shown before the decision [frame 751](../../screenshots/Slack%20web%20Jul%202024%20751.png).
...
| Notification preference | One of all messages, mentions or off, plus a separate mute flag; surfaced both as a four-option menu and as a radio group | [frame 89](../../screenshots/Slack%20web%20Jul%202024%2089.png), [frame 90](../../screenshots/Slack%20web%20Jul%202024%2090.png), [frame 96](../../screenshots/Slack%20web%20Jul%202024%2096.png) |
| Per-device notification override | An optional nested scope applied to mobile devices only, disclosed by its own checkbox | [frame 93](../../screenshots/Slack%20web%20Jul%202024%2093.png), [frame 94](../../screenshots/Slack%20web%20Jul%202024%2094.png) |
| Thread-reply notification flag | A per-channel checkbox covering all thread replies in the channel | [frame 92](../../screenshots/Slack%20web%20Jul%202024%2092.png) |
| Starred flag | A toggle whose set state fills and tints the control and relocates the channel into a starred sidebar group | [frame 86](../../screenshots/Slack%20web%20Jul%202024%2086.png), [frame 87](../../screenshots/Slack%20web%20Jul%202024%2087.png) |
...
| Joined state | A per-viewer marker rendered on a browser row as a check glyph and a joined label in the accent colour, absent on the archived row | [frame 125](../../screenshots/Slack%20web%20Jul%202024%20125.png), [frame 133](../../screenshots/Slack%20web%20Jul%202024%20133.png) |
...
| Sidebar placement | Membership of a sidebar group — channels, starred, or a user-created section — rendered as grouping and nesting | [frame 86](../../screenshots/Slack%20web%20Jul%202024%2086.png), [frame 120](../../screenshots/Slack%20web%20Jul%202024%20120.png) |
...
| `E-PLAN` | Per-capability entitlement scoped to a channel: external invitation at creation time, and two Integrations capabilities whose only action is an upgrade action | [frame 69](../../screenshots/Slack%20web%20Jul%202024%2069.png), [frame 62](../../screenshots/Slack%20web%20Jul%202024%2062.png), [frame 83](../../screenshots/Slack%20web%20Jul%202024%2083.png) |
...
| `E-MESSAGE` | Author, timestamp and body, with system messages authored by the acting person and app-posted messages carrying an app author and a workflow badge; full model in [03-messaging-and-composer.md](03-messaging-and-composer.md) | [frame 100](../../screenshots/Slack%20web%20Jul%202024%20100.png), [frame 120](../../screenshots/Slack%20web%20Jul%202024%20120.png) |
...
erDiagram
    ...
    E-CHANNEL ||--o| E-NOTIFICATION : "is overridden by"
    E-CHANNEL }o--o{ E-EXTERNAL-ORG : "admits people from"
    E-PLAN ||--o{ E-CHANNEL : "gates capabilities of"
    E-INVITATION }o--|| E-EXTERNAL-ORG : "is addressed to"
    E-APP ||--o{ E-MESSAGE : "posts"
    E-USER ||--o{ E-MESSAGE : "authors"
```

The cardinality on each edge is the one the frames support. A channel has exactly one creator [frame 81](../../screenshots/Slack%20web%20Jul%202024%2081.png) and many members, and a person belongs to many channels, so membership is many-to-many while creation is one-to-many. A channel carries at most one notification override, because the setting is a single scope with a single mute flag rather than a list [frame 89](../../screenshots/Slack%20web%20Jul%202024%2089.png), [frame 92](../../screenshots/Slack%20web%20Jul%202024%2092.png). An invitation is scoped to at most one channel, which is exactly what the disambiguation dialog's own sub-label says [frame 65](../../screenshots/Slack%20web%20Jul%202024%2065.png).
...
- **The rename system message renders the prior channel name in two different forms.** Within this document's own frames the prior name is **unhyphenated** at [frame 341](../../screenshots/Slack%20web%20Jul%202024%20341.png), [frame 371](../../screenshots/Slack%20web%20Jul%202024%20371.png), [frame 373](../../screenshots/Slack%20web%20Jul%202024%20373.png), [frame 393](../../screenshots/Slack%20web%20Jul%202024%20393.png) and [frame 545](../../screenshots/Slack%20web%20Jul%202024%20545.png), all timestamped alike, and **hyphenated** at [frame 100](../../screenshots/Slack%20web%20Jul%202024%20100.png), which carries a different timestamp. The same two forms appear at the secondary frames [frame 550](../../screenshots/Slack%20web%20Jul%202024%20550.png) and [frame 200](../../screenshots/Slack%20web%20Jul%202024%20200.png) respectively. Because [frame 134](../../screenshots/Slack%20web%20Jul%202024%20134.png) shows **four** rename messages in one history, whether these are one event rendered two ways or two separate renames is **not determinable from the pixels**, and no reconciliation is offered. The consequence for a build is the same either way: echo the stored prior name verbatim and never normalise it.
...
Two duplicate groups fall inside this document's frame set, and each is treated as a **repeated state rather than a transition**.
...
- [ ] `E-CHANNEL` persists every field listed in **Implied data model**, and the notification preference, starred flag and archived state are per-channel rather than global [frame 86](../../screenshots/Slack%20web%20Jul%202024%2086.png), [frame 89](../../screenshots/Slack%20web%20Jul%202024%2089.png), [frame 134](../../screenshots/Slack%20web%20Jul%202024%20134.png).
