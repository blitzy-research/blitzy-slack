...
This area met **fourteen recurring structures** that no contract covered when it was written. All fourteen were reported for central definition rather than forced onto an identifier they did not satisfy, and **every one is now resolved** in [00-product-overview.md](00-product-overview.md) — ten as contracts of their own and four as instances of contracts that already existed. This document defines none of them and references each by identifier in the table above.
...
6. **Two preconditions are visible on the form, and the corpus disambiguates them only partly.** **Inferred:** the *enabling* precondition for the primary action is content in the input rather than completion of the verification widget, because the widget is unticked in **both** the disabled capture and the available capture, and the only difference between the two frames is the input's content and the counter that reports it — this is the same reading published in `21-states.md` [frame 950](../../screenshots/Slack%20web%20Jul%202024%20950.png), [frame 951](../../screenshots/Slack%20web%20Jul%202024%20951.png). **The verification widget's role is left unspecified, because the corpus does not evidence it.** What is observed is its placement inside the form and that it is unticked in both captures; **no frame captures a submission attempt**, so whether it is enforced at submit time, whether it is enforced at all and what it would report on failure are all uncaptured. A build must not read a requirement out of the widget's mere presence: it decides the control's role for itself under `S-GAP` and records the decision, and it does not treat this document as having specified one.
...
- [ ] The article-feedback form renders a **human-verification widget inside the form**, between the counter and the primary action, and the widget is **unticked in both captured states** — the disabled one and the available one [frame 950](../../screenshots/Slack%20web%20Jul%202024%20950.png), [frame 951](../../screenshots/Slack%20web%20Jul%202024%20951.png). **What the widget does at submission time is not captured**: no frame shows a submission attempt, so no submit-time behaviour is specified here and none may be assumed from the widget's presence
