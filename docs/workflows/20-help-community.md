...
**This area owns no entity, and contributes to five that other areas own.** Its subject is published content and support correspondence rather than the product's own domain objects, so it owns no *product* entity — what it does own authoritatively is **one variant of an entity another area owns**: the public help search is the **unauthenticated, public-corpus variant of `E-SEARCH-QUERY`**, no other area specifies it, and this document is therefore the authority for that variant's field set. **Variant specified here:** the public help-corpus variant of `E-SEARCH-QUERY`, owned by [09-search-and-filters.md](09-search-and-filters.md). **Contributed to, additively:** `E-PREFERENCE`, `E-USER`, `E-PLAN` and `E-WORKSPACE`, each owned elsewhere — which with the `E-SEARCH-QUERY` variant above makes five. No identifier is coined here that the [master index](README.md) does not already carry — its entity list is fixed at twenty — and the two populations are kept apart deliberately: what this area specifies is separated from what it merely reports upward, so a build can tell which document to treat as authoritative for each field.

### The public search variant specified by this area

| Entity | Observed fields | Evidence |
|---|---|---|
| `E-SEARCH-QUERY` — **public help-corpus variant** | A **distinct variant of `E-SEARCH-QUERY`**, which [09-search-and-filters.md](09-search-and-filters.md) owns for the in-product, workspace-scoped, authorization-filtered search. This variant is a public query over the help-article corpus, discriminated from the in-product one by its **corpus scope**, and it shares no field with it beyond the query string: a **query string**; a **total-result count**; **per-result match highlighting**; a **per-result category label**; and an **overflow affordance carrying a count-with-plus notation**. It has **no result-type tabs, no facets, no sort, no pagination, no workspace scope and no per-viewer authorization filter**, because its corpus is public. **The variant discriminator is a security boundary, not a convenience:** the two variants must be served by **two separate read paths**, and a public query may never be resolved by the workspace-scoped path — attaching a public, unauthenticated query to the reads that carry workspace content is precisely the confusion `S-AUTHZ-READ` exists to prevent | [frame 944](../../screenshots/Slack%20web%20Jul%202024%20944.png), [frame 945](../../screenshots/Slack%20web%20Jul%202024%20945.png) |
...
- [ ] The public help search resolves the **public help-corpus variant of `E-SEARCH-QUERY`** over the public article corpus through its own read path and **never** touches the workspace-scoped variant's path, so no unauthenticated query can reach workspace content [frame 944](../../screenshots/Slack%20web%20Jul%202024%20944.png), [frame 945](../../screenshots/Slack%20web%20Jul%202024%20945.png)
