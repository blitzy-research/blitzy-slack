...
### Two component variants reported to `00-product-overview.md`

Both are recorded here as findings and **not defined here**, because component contracts have exactly one home.

1. **`C-DROPDOWN-MENU` with a nested submenu.** The account menu is anchored to a persistent control — the account avatar — which is the dropdown's distinguishing property, yet it carries submenus that open sideways, which the contract currently attributes only to `C-CONTEXT-MENU` [frame 504](../../screenshots/Slack%20web%20Jul%202024%20504.png), [frame 514](../../screenshots/Slack%20web%20Jul%202024%20514.png), [frame 517](../../screenshots/Slack%20web%20Jul%202024%20517.png). It also carries a **text input** inside a menu row, which neither contract records. The closest identifier is used above; the variant needs adding to the inventory in [00-product-overview.md](00-product-overview.md).
2. **`C-DATE-PICKER-POPOVER` month-grid mode.** Activating the picker's month-and-year control replaces the day grid with a three-column grid of twelve months and narrows the header to a year stepper [frame 528](../../screenshots/Slack%20web%20Jul%202024%20528.png). The contract records the day grid and the month-and-year control but not this second mode.
