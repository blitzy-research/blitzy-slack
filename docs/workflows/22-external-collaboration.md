...
**What this document does not own.** The workspace invite modal and its journey, the invite-link mechanism and its own separate expiry, and the guest-account invite wizard belong to [01-onboarding-and-auth.md](01-onboarding-and-auth.md). The add-people modal, channel creation and the three-dialog channel-invitation sequence belong to [02-channels.md](02-channels.md). The direct-message conversation that an acceptance materialises belongs to [05-direct-messages.md](05-direct-messages.md). The account-type badge that marks an external person in a conversation list belongs to [13-profiles-people.md](13-profiles-people.md). The administered surfaces the management rows open belong to [15-admin-workspace.md](15-admin-workspace.md). Every reusable component contract referenced here is defined once in [00-product-overview.md](00-product-overview.md) and is never restated.
...
> **Partial capture:** all three management rows and all three settings-menu entries are **entry points only**. No frame shows what any of them opens — there is no requests list, no per-request detail, no approve or decline control, no approval confirmation, no connections table and no permissions form anywhere in the corpus. Their destinations are administered surfaces owned by [15-admin-workspace.md](15-admin-workspace.md), and an approval screen that cannot be cited does not exist for this catalog.
...
An authenticated session on the external-connections destination. **No plan entitlement is required for this path** — unlike the create-channel action beside it, the start-a-conversation action carries no plan-tier badge at either capture [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png), [frame 495](../../screenshots/Slack%20web%20Jul%202024%20495.png). The recipient need not already have an account: the flow's whole premise is that the invitation travels by email to an address.
...
**A recurring structure with no identifier yet, reported rather than defined.** The testimonial panel is a **paged content carousel**: a single item rendered at a time, a directional chevron at one edge, and a row of pagination indicators whose filled position tracks the item on show, with the forward affordance suppressed at the end of the set and the back affordance at its start [frame 496](../../screenshots/Slack%20web%20Jul%202024%20496.png), [frame 497](../../screenshots/Slack%20web%20Jul%202024%20497.png). It is not one of the identifiers defined in [00-product-overview.md](00-product-overview.md), and it is **not defined here** — this catalog defines every component contract in exactly one place. It is reported to [00-product-overview.md](00-product-overview.md) for definition, with the note that the same structure already appears inside that document's `C-DETAILS-PANE` contract as a card carousel with a pager indicator, so a shared identifier is likely warranted rather than a local description. Until one exists, the structure is specified here descriptively and a build should treat the two occurrences as one component.
...
One diagram covers the area's cross-actor arithmetic: how an inviting user, the client, the workspace service and a person at another organization interact; where a conditional administrator sign-off enters; and exactly where the corpus stops. Every participant and message corresponds to something a cited frame shows, and the two steps the corpus does not capture are labelled as unobserved rather than drawn as if they were.

```mermaid
sequenceDiagram
    autonumber
    actor Inviter as Inviting user
    participant Web as Web client
    participant Svc as Workspace service
    actor Admin as Approving administrator
    actor Invitee as External person
    ...
    Web->>Svc: Create the externally-scoped invitation record [frames 500, 503]
    opt Where the organization requires sign-off
        Svc->>Admin: Raise the invitation for review and approval [frames 494, 502]
    end
    Svc-->>Invitee: Deliver the invitation by email [frame 498]
    Web-->>Inviter: Confirm the send and state a 14-day acceptance window [frame 500]
    Note over Svc,Invitee: The window is stated as 14 days; the sent record renders it as a concrete expiry date [frames 500, 503]
    ...
    alt Invitee accepts inside the window - promised on the sender's screens, never captured
        Svc-->>Inviter: Notify the sender and add the person to the direct-message list [frames 498, 500]
...
| Entry point, ungated | The start-a-conversation action, and the create-channel action inside the card, both carrying no badge | [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png) |
...
Two entities are **owned here**, one is modelled only as far as the pixels reach, and three receive contributions that the [master index](README.md) aggregates additively. Every field cites the frame that shows it.
...
| Per-channel permission level, held against the **organization** rather than the person | The permissions dialog is headed with what external people *from this organization* can do, and offers post-and-invite or only-post | [frame 66](../../screenshots/Slack%20web%20Jul%202024%2066.png) |
...
**The acceptance window belongs to this entity.** The fourteen-day window is a property of the **pending relationship** rather than of a shareable link, and the [master index](README.md) models it on `E-EXTERNAL-ORG` for exactly that reason [frame 500](../../screenshots/Slack%20web%20Jul%202024%20500.png). This document does not contradict that placement; it records the window here and the per-record expiry date under the invitation below.
...
|---|---|---|
| `E-USER` | An account type that distinguishes an external or guest person from a member, rendered as a badge beside the display name on conversation rows — `guest` is observed on rows in two separate captures. Searchability of an external person by name, company or email address. The area does **not** own the badge; [13-profiles-people.md](13-profiles-people.md) does | [frame 45](../../screenshots/Slack%20web%20Jul%202024%2045.png), [frame 65](../../screenshots/Slack%20web%20Jul%202024%2065.png), [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png) |
...
| `E-PLAN` | A gated capability: creating a channel shared with an external organization is marked with an inline paid-tier badge on its entry point, while starting a one-to-one external conversation is not. The plan entity is owned by [18-pricing-plans.md](18-pricing-plans.md) | [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png) |
...
| To | Via | Owner | Frames |
|---|---|---|---|
| Administered management surfaces, outside the application | Any of the three management rows or the settings menu's entries, each marked with an external-link affordance on at least one of the two lists | [15-admin-workspace.md](15-admin-workspace.md) | [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png), [frame 495](../../screenshots/Slack%20web%20Jul%202024%20495.png) |
...
| Creating a channel shared with an external organization is plan-gated, and the gate is disclosed rather than hidden | An inline badge naming a paid plan tier sits on the header's create-channel action; the action is still rendered and still reachable | [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png) |
| Starting a one-to-one external conversation is **not** plan-gated | The start-a-conversation action carries no badge at either capture of the destination | [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png), [frame 495](../../screenshots/Slack%20web%20Jul%202024%20495.png) |
...
| Management destinations are separate surfaces outside the application | Every management row carries an external-link affordance at its trailing edge | [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png) |
...
| An external permission level is held against the **organization**, not the individual | The permissions dialog is headed with what external people *from this organization* can do | [frame 66](../../screenshots/Slack%20web%20Jul%202024%2066.png) |
...
- **Do not derive the expiry date by assuming the two captures describe the same invitation.** Store the window and derive the date; the corpus never shows a send date and its expiry date together [frame 500](../../screenshots/Slack%20web%20Jul%202024%20500.png), [frame 503](../../screenshots/Slack%20web%20Jul%202024%20503.png).
- **Do not attach the external permission level to a person.** It is stated against the organization, so a second person from the same organization inherits it rather than being configured separately [frame 66](../../screenshots/Slack%20web%20Jul%202024%2066.png).
- **Do not assume the plan gate applies to the whole area.** One of the two starting actions is badged and the other is not, in the same capture [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png).
...
- [ ] The create-channel action in the header carries an inline badge naming a paid plan tier, and the start-a-conversation action carries none [frame 494](../../screenshots/Slack%20web%20Jul%202024%20494.png).
...
- [ ] The expiry date on an outbound record is derived from the stored acceptance window rather than hard-coded, and the fourteen-day window is never confused with the separate nineteen-day expiry of a workspace invite link [frame 500](../../screenshots/Slack%20web%20Jul%202024%20500.png), [frame 503](../../screenshots/Slack%20web%20Jul%202024%20503.png), [frame 46](../../screenshots/Slack%20web%20Jul%202024%2046.png).
...
- [ ] An external permission level is set for **external people from that organization** rather than per person, offering post-and-invite with full access and a retained copy of the channel history, or only-post without workflows, apps or coworker invitation [frame 66](../../screenshots/Slack%20web%20Jul%202024%2066.png).
...
**Frames this document cites as evidence but does not own.** Seven, all secondary cross-references excluded from the coverage arithmetic by design, because another area owns and specifies each: [frame 399](../../screenshots/Slack%20web%20Jul%202024%20399.png) (the rail's overflow menu, where this area's entry sits after a separator rule) and [frame 340](../../screenshots/Slack%20web%20Jul%202024%20340.png) (the sidebar filter's external-people scope), owned by [00-product-overview.md](00-product-overview.md); [frame 45](../../screenshots/Slack%20web%20Jul%202024%2045.png) (the dismissible external advisory), [frame 46](../../screenshots/Slack%20web%20Jul%202024%2046.png) (the invite-link expiry, named here only to keep it separate from the acceptance window) and [frame 49](../../screenshots/Slack%20web%20Jul%202024%2049.png) (the invite-as select's external path), owned by [01-onboarding-and-auth.md](01-onboarding-and-auth.md); and [frame 65](../../screenshots/Slack%20web%20Jul%202024%2065.png), [frame 66](../../screenshots/Slack%20web%20Jul%202024%2066.png) and [frame 67](../../screenshots/Slack%20web%20Jul%202024%2067.png) (the channel-scoped external invitation's disambiguation, permission and send steps), owned by [02-channels.md](02-channels.md).

**On the absence of project rules.** No user-specified rules exist for this project — the rules document returns that none were provided — so no rule governs this document and none is invented here. Their absence did not lower the bar: this area was held instead to the enterprise practices the [master index](README.md) sets out, which is why every claim above carries a frame citation, why every inference is marked and given its basis, why four inconsistencies are recorded rather than reconciled, why no component contract is restated, and why every uncaptured surface is declared as a gap instead of being described from anything other than the pixels.
