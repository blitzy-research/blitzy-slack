...
> **Build obligation:** **what the corpus shows about a payment instrument is a field set and a masked rendering, and nothing more — so nothing more is specified.** Observed: the form captures a number, an expiry and a security code [frame 678](../../screenshots/Slack%20web%20Jul%202024%20678.png); the stored record retains a masked identifier, the network, the expiry and a default flag, renders the number's digits masked, and states that future payments will use it [frame 679](../../screenshots/Slack%20web%20Jul%202024%20679.png); and the purchase form captures the same three fields [frame 611](../../screenshots/Slack%20web%20Jul%202024%20611.png). The obligations that follow are exactly the two the evidence supports: per `S-SECRET` **no stored instrument value is ever rendered in full or transcribed** — this document transcribes none — and per `S-AUTHZ-OP` **adding, changing or removing an instrument is authorized server-side** like every other billing mutation. **How an instrument is transmitted, delegated, stored or logged is not observable in any frame, so this catalog states no rule for it**; `S-GAP` applies, and the build decides that design and records it as its own. `E-WORKSPACE`'s payment fields mean the retained field set above and nothing beyond it.
...
- [ ] A stored instrument retains **only** a masked identifier, the network, the expiry and a default flag, and **no stored value is ever rendered in full** on any surface [frame 678](../../screenshots/Slack%20web%20Jul%202024%20678.png), [frame 679](../../screenshots/Slack%20web%20Jul%202024%20679.png). How an instrument is transmitted, delegated, stored or logged is **not evidenced anywhere in the corpus**, so this catalog specifies none of it: the build decides that design and records it as its own under `S-GAP`.
