...
Every item below is a requirement the corpus **cannot** evidence either way, written in the obligation register defined in [00-product-overview.md](00-product-overview.md) and resolving to the `S-*` contracts defined there. **Adding third-party software to a workspace is the operation in this catalog whose consequences the corpus reveals least**, which is why the gap between what the frames show and what a build must decide is at its largest here — and why the obligations below stop at what the frames support instead of filling that gap with a described screen.

> **Build obligation:** **installation is a privileged operation offered from a surface that is readable without a session, and everything that happens after it is activated is uncaptured.** What the corpus shows is the *offer*: the add affordance is placed on an installed-app row and a recommended row on the in-product apps surface [frame 400](../../screenshots/Slack%20web%20Jul%202024%20400.png), on a row of the automations-scoped variant of it [frame 368](../../screenshots/Slack%20web%20Jul%202024%20368.png), and as an add-to-workspace action on the directory site's detail page, which is fully readable **while signed out** [frame 370](../../screenshots/Slack%20web%20Jul%202024%20370.png), [frame 921](../../screenshots/Slack%20web%20Jul%202024%20921.png). It also shows that permissions are a **directory tab whose contents are never opened**, and that **no authorization screen, consent screen or permission-grant screen appears anywhere in this area** [frame 370](../../screenshots/Slack%20web%20Jul%202024%20370.png). Per `S-AUTHZ-OP` the one obligation that follows from the observed offer is that installation — and any later change to an installation — is authorized **server-side against an authenticated, authorized principal**: reaching a signed-out catalogue page is not a capability, so the add action there resolves to authentication and then to an authorization check rather than to an install.
>
> **What this document deliberately does not specify, because the corpus does not evidence it.** The **outcome** of activating the add affordance; the **permissions or capabilities an app requests or is granted**, which exist here only as an unopened tab label; any **consent, review or grant step** and anything it would record; and any **later change to an installation** — removal, reconfiguration or a change of what an app may do. `S-GAP` governs each of these: it obliges the build to decide and record its own design, and it **forbids this catalog from describing a screen it never saw**, so no screen, no state and no entity field is asserted for any of them here. `E-APP`, as modelled in this document, carries no permission, scope, consent, installer or install-timestamp field, and none is inferred into it.
...
> **Build obligation:** **`S-GAP` applies to the states and surfaces this area does not evidence**, and the set is exactly the set of absences this document records rather than a set of screens imagined for it — an apps surface with nothing installed, a post-add row or card, an applied filter or facet, a loading state, the outcome of the add affordance, and the app-removal, app-reconfiguration and app-settings surfaces the corpus names nowhere. Renderings come from the state matrix in [21-states.md](21-states.md); behaviour comes from the `S-*` contracts. Naming a gap here obliges the build to design it and record the decision; it does not license this document to describe it.
