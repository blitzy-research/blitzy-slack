...
| `C-FILTER-CHIP` — **underlined-select rendering and active-count trigger** | Folded into **`C-FILTER-CHIP`** — as two rendering variants: an underlined label with a trailing caret and a dotted focus ring when open, and a trigger carrying a parenthesised active count | Two renderings the contract does not record: a trigger drawn as an underlined label with a trailing caret and a dotted focus ring when open, and a trigger carrying a parenthesised count of pending or applied filters | [frame 840](../../screenshots/Slack%20web%20Jul%202024%20840.png), [frame 853](../../screenshots/Slack%20web%20Jul%202024%20853.png) |
...
| `C-TEMPLATE-CARD` — **typed content-card form** | Folded into **`C-TEMPLATE-CARD`** — as its typed content-card form — an image slot, a content-type eyebrow, a headline, an optional description and a call to action whose wording is a function of the type | An image slot, a content-type eyebrow, a headline, an optional description and a call-to-action link **whose wording is a function of the type**, where the eyebrow may be replaced by a marker pair of a date badge with a virtual label or a play glyph with an on-demand label. The image slot and the description are independently optional and are each observed absent | [frame 757](../../screenshots/Slack%20web%20Jul%202024%20757.png), [frame 847](../../screenshots/Slack%20web%20Jul%202024%20847.png), [frame 867](../../screenshots/Slack%20web%20Jul%202024%20867.png), [frame 871](../../screenshots/Slack%20web%20Jul%202024%20871.png) |
...
These are **content structures a publishing system must model, not product entities**. None is given an `E-*` identifier **here**: the entity list is owned by the [master index](README.md), and inventing an identifier locally would break that ownership. The master index does aggregate them, and it is worth stating which row becomes which identifier so the two documents demonstrably agree: the editorial item, editorial article, customer story, collection and event or webinar rows aggregate into `E-CONTENT-ITEM`; the career opportunity row into `E-JOB-POSTING`; the merchandise product row into `E-STORE-PRODUCT`; the basket and billing-address rows into `E-BASKET`; the service health record into `E-SERVICE-STATUS`; and the person captured by the three lead-capture forms into `E-LEAD`. The remaining rows — the two facet taxonomies, the policy document and the statistical claim — are content-management concerns the master index deliberately leaves unidentified, and this document is the only place they are specified.
