...
**This area owns no *product* entity — and it does own six *content and commerce* entities.** Its subject is published marketing and editorial content, a merchandise catalogue, a public service-health record and the people who submit its forms; none of those is one of the product's own domain objects, and all six are nevertheless entities, with this document as the authority for their field sets. **Owned here:** `E-CONTENT-ITEM`, `E-LEAD`, `E-JOB-POSTING`, `E-SERVICE-STATUS`, `E-STORE-PRODUCT` and `E-BASKET`, whose fields are specified in the **Page content structures** section below and aggregated by the [master index](README.md), which records this document as their owning area. Everything in the table that follows is a **contribution** to an entity another area owns, made additively; **no new identifier is coined anywhere in this document**, because the identifier list itself belongs to the master index.
