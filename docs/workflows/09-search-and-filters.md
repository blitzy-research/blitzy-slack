...
Every node below corresponds to a state observed in a cited frame, and nothing is a plausible route that the corpus does not show. The zero-result branch is included because the corpus captures it directly [frame 692](../../screenshots/Slack%20web%20Jul%202024%20692.png). **Two link styles carry two different claims**, and the distinction is evidential rather than decorative: a **solid edge** is a transition the corpus captures on both sides, while a **dotted, labelled edge** means the affordance is rendered but **neither the activation nor the destination is captured anywhere**. The two edges into `OBJECT` are the only dotted ones, and the paragraph beneath the diagram states exactly what is missing behind them.
...
flowchart TD
    ...
    SORTMENU --> SORTED
    SORTED --> GRID
    GRID -.->|"activation and destination not captured"| OBJECT
    RESULTS -.->|"activation and destination not captured"| OBJECT
```

The diagram is deliberately silent about how a result is opened, because no frame captures the transition: result rows and cards are rendered throughout the area but no capture shows one activated or the destination it leads to. That is why **both edges into `OBJECT` are drawn dotted** rather than solid — the node exists only to name where those edges point, and the destination surfaces are specified by the area documents listed in **Transitions in and out**. A build therefore designs the opening behaviour rather than transcribing it, as the partial capture beside **Transitions in and out** records.
...
| `C-TYPEAHEAD-PANEL` | The overlay's completion panel — three sections of query suggestions, entity matches then scoped message previews, narrowing as characters are typed. Reported from this area's own candidate list and contracted in [00-product-overview.md](00-product-overview.md) [frame 687](../../screenshots/Slack%20web%20Jul%202024%20687.png) |
| `C-PAGER` | The centred pagination row beneath the result grid — an inert previous control, a filled current-page indicator, then a next control; absent from the same results rendered as a list |

No component contract is restated here. One structure this area shows had no identifier when it was written — the pager beneath the result grid — so it was reported to [00-product-overview.md](00-product-overview.md) rather than contracted locally. **It is now defined there as `C-PAGER`** and is referenced by identifier [frame 704](../../screenshots/Slack%20web%20Jul%202024%20704.png). The overlay's own completion panel is likewise now contracted there as `C-TYPEAHEAD-PANEL` [frame 687](../../screenshots/Slack%20web%20Jul%202024%20687.png), so this document references both by identifier and restates neither.
...
This area owns **`E-SEARCH-QUERY`**. Every field below is claimed only where a frame shows it, and every row cites that frame. Every field aggregates into the consolidated model in the [Workflow Catalog](README.md).
...
Every item below is a requirement the corpus **cannot** evidence either way, written in the obligation register defined in [00-product-overview.md](00-product-overview.md) and resolving to the `S-*` contracts defined there. **Retrieval is the surface with the widest blast radius in the product**: it reaches across every conversation at once, which is exactly why its scoping cannot be left to the reader.

> **Build obligation:** **every result, every count, every facet value and every typeahead preview is computed after applying the searcher's read authorization — and a count is not an exception.** The surface renders a result-type tab bar carrying **one integer per type, recomputed over the whole set whenever a constraint changes and rendered even when zero** [frame 694](../../screenshots/Slack%20web%20Jul%202024%20694.png), [frame 700](../../screenshots/Slack%20web%20Jul%202024%20700.png); a typeahead that surfaces matching channels, canvases and query suggestions **while the query is still being typed** [frame 687](../../screenshots/Slack%20web%20Jul%202024%20687.png); facet lists such as the file-type filter's eight options [frame 696](../../screenshots/Slack%20web%20Jul%202024%20696.png); and result rows carrying names, sharers, dates and previews [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), [frame 704](../../screenshots/Slack%20web%20Jul%202024%20704.png). Per `S-AUTHZ-READ` each of those is computed over the **searcher's authorized set** and recomputed on every render. Three consequences, and the first is the one an implementation most often gets wrong. **A count is a disclosure oracle**: a per-type integer computed over content the searcher may not read reveals that the content exists — and, with a few refined queries, roughly what it says — without ever rendering a row, so counts, facets and totals are computed over the authorized set only and **never reveal the existence of unauthorized content**. **A typeahead is a projection per keystroke**: it is scoped identically, because an unscoped typeahead leaks a match on the first characters of a term. And **a scope chip is not an authorization boundary**: the my-channels-only constraint is an **optional, default-off** filter [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), so it narrows a result set for convenience and must never be the mechanism by which unauthorized content is excluded.

> **Build obligation:** **the query language needs a grammar, and token values are re-authorized server-side.** Queries carry modifier tokens — `from:`, `in:`, `with:`, type and date constraints — that are both written by ticking popover options and typed by hand into the same field [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), [frame 698](../../screenshots/Slack%20web%20Jul%202024%20698.png), [frame 701](../../screenshots/Slack%20web%20Jul%202024%20701.png), and this document records that the binding direction between the two is not observable. Per `S-CONTENT`: the grammar is **explicit**, so what is a token and what is literal text is decided by the parser rather than by chance; a **literal colon in search text is escapable** and does not become a token; the **number of repeatable tokens is bounded**, as is total query length; matching is **literal rather than pattern-based**, so a query cannot be interpreted as an expression and cannot be made to backtrack catastrophically; and **every token value is re-authorized server-side** — a hand-typed `in:` or `from:` naming a conversation or a person the searcher may not read yields nothing rather than an error that confirms the name exists.

> **Build obligation:** **match highlighting is a sink where both sides are untrusted.** The needle is typed by the searcher and the haystack is authored by someone else, and the surface highlights matched terms inside result rows and previews [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), [frame 704](../../screenshots/Slack%20web%20Jul%202024%20704.png). Per `S-CONTENT` **both are encoded before any highlight markup is inserted, never after**; matching is literal; and the term is length-bounded before matching begins.

> **Build obligation:** **stored queries are author-scoped, bounded and deletable.** Recent queries persist across sessions and are re-offered in the overlay [frame 686](../../screenshots/Slack%20web%20Jul%202024%20686.png), and a stored query is not neutral text: it can carry `from:` and `in:` tokens naming colleagues and private conversations [frame 701](../../screenshots/Slack%20web%20Jul%202024%20701.png). Per `S-SECRET` and `S-AUTHZ-READ`: a stored query is **readable only by its author**, never projected to anyone else and never included in an export; it carries a **stated retention bound** rather than persisting indefinitely; and the person has a **deletion path** for individual entries and for the whole history. This document records that deletion is not verified anywhere in the corpus, which is why it is stated as an obligation rather than as an observation.

> **Build obligation:** **the people tab is a directory-scale enumeration surface over personal fields.** It filters people by **title and location** and renders name, avatar, presence and profile details at directory scale [frame 490](../../screenshots/Slack%20web%20Jul%202024%20490.png), [frame 491](../../screenshots/Slack%20web%20Jul%202024%20491.png), [frame 493](../../screenshots/Slack%20web%20Jul%202024%20493.png). Per `S-AUTHZ-READ` each field is projected only to a viewer the **per-field audience model permits** — the model owned by [15-admin-workspace.md](15-admin-workspace.md), which is the surface that decides whether a field exists at all — and per `S-EXPORT` the filter is **server-side rate-limited** and result counts are capped, because a filter that returns a page at a time enumerates the whole directory given enough requests.

> **Build obligation:** **the feedback control's payload is specified or it transmits nothing.** A feedback affordance sits on a surface that holds the searcher's query and its full filter state [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), and its destination and payload are not captured. Per `S-TELEMETRY` — the payload contract, **not** the device-capture one, and the distinction matters here because a search query is the most revealing string a person types: the build either states exactly what is transmitted, assembling it from a declared field allowlist that **excludes the query string, the token values, the filter state and any result content**, or transmits nothing beyond a message the person composed; the destination is bounded rather than an automatic upload; and per `S-RETENTION` the stored payload carries a stated retention bound and a deletion path either way.

> **Build obligation:** **`S-GAP` applies to the states this area does not evidence** and the build designs each using the contracts named beside it: a result set that is loading, a retrieval that failed, a query rejected for length or syntax, a token naming something the searcher may not read, a rate limit reached, and a result whose containing conversation the viewer has lost access to since the result was computed. The zero-result state is the one this area does supply [frame 692](../../screenshots/Slack%20web%20Jul%202024%20692.png), and it is the canonical exemplar recorded in the state matrix in [21-states.md](21-states.md).
...
### Retrieval scoping, query safety and stored-query privacy
...
- [ ] Every result row, **every per-type count**, every facet value and every typeahead preview is computed **after** applying the searcher's read authorization, and is recomputed on every render, per `S-AUTHZ-READ` [frame 694](../../screenshots/Slack%20web%20Jul%202024%20694.png), [frame 687](../../screenshots/Slack%20web%20Jul%202024%20687.png), [frame 696](../../screenshots/Slack%20web%20Jul%202024%20696.png).
- [ ] **A count never reveals the existence of content the searcher may not read**: per-type integers, facet counts and totals are computed over the authorized set only, including when they fall to zero and the tab stays rendered [frame 694](../../screenshots/Slack%20web%20Jul%202024%20694.png), [frame 700](../../screenshots/Slack%20web%20Jul%202024%20700.png).
- [ ] The my-channels-only constraint is treated as an **optional convenience filter and never as the authorization boundary**, because it is default-off [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png).
- [ ] The query language has an **explicit grammar**: a literal colon in search text is escapable and does not become a token, repeatable tokens and total query length are bounded, matching is **literal rather than pattern-based**, and **every token value is re-authorized server-side** so a hand-typed `in:` or `from:` naming something the searcher may not read yields nothing rather than an error confirming the name exists [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), [frame 698](../../screenshots/Slack%20web%20Jul%202024%20698.png), [frame 701](../../screenshots/Slack%20web%20Jul%202024%20701.png).
- [ ] Match highlighting **encodes both the term and the surrounding text before inserting highlight markup**, matches literally, and bounds the term's length, per `S-CONTENT` [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png), [frame 704](../../screenshots/Slack%20web%20Jul%202024%20704.png).
- [ ] A stored query is **readable only by its author**, never projected to anyone else and never exported, carries a stated retention bound, and has a deletion path for individual entries and for the whole history — which matters because a stored query can name colleagues and private conversations in its tokens, per `S-SECRET` [frame 686](../../screenshots/Slack%20web%20Jul%202024%20686.png), [frame 701](../../screenshots/Slack%20web%20Jul%202024%20701.png).
- [ ] The people tab projects each personal field only to a viewer the **per-field audience model permits** — the model owned by [15-admin-workspace.md](15-admin-workspace.md) — and its filter is **server-side rate-limited** with result counts capped, per `S-EXPORT` [frame 490](../../screenshots/Slack%20web%20Jul%202024%20490.png), [frame 491](../../screenshots/Slack%20web%20Jul%202024%20491.png), [frame 493](../../screenshots/Slack%20web%20Jul%202024%20493.png).
- [ ] The feedback control either states exactly what it transmits — **excluding the query string, the token values, the filter state and all result content** — or transmits nothing beyond a message the person composed, with a stated retention bound either way [frame 689](../../screenshots/Slack%20web%20Jul%202024%20689.png).
- [ ] Every state this area does not evidence — a loading result set, a failed retrieval, a query rejected for length or syntax, a token naming something unreadable, a rate limit reached, and a result whose containing conversation the viewer has since lost access to — is designed and implemented per `S-GAP` using the state matrix in [21-states.md](21-states.md); the zero-result state is the one this area supplies [frame 692](../../screenshots/Slack%20web%20Jul%202024%20692.png).
...
**Frames this document cites as evidence but does not own.** Eleven, all secondary cross-references excluded from the coverage arithmetic by design. Three were added by this document's security obligations — [frame 490](../../screenshots/Slack%20web%20Jul%202024%20490.png), [frame 491](../../screenshots/Slack%20web%20Jul%202024%20491.png) and [frame 493](../../screenshots/Slack%20web%20Jul%202024%20493.png), owned by [13-profiles-people.md](13-profiles-people.md) and cited because the people destination is where the directory-scale projection of personal fields is actually rendered, which is the surface the people-tab obligation constrains. The other eight are: [frame 550](../../screenshots/Slack%20web%20Jul%202024%20550.png) and [frame 450](../../screenshots/Slack%20web%20Jul%202024%20450.png), owned by [00-product-overview.md](00-product-overview.md) and [08-lists.md](08-lists.md), cited for the resting search entry and its workspace-naming placeholder; [frame 683](../../screenshots/Slack%20web%20Jul%202024%20683.png) and [frame 705](../../screenshots/Slack%20web%20Jul%202024%20705.png), owned by [15-admin-workspace.md](15-admin-workspace.md) and [20-help-community.md](20-help-community.md), cited only as the visual evidence for this area's two boundaries; [frame 944](../../screenshots/Slack%20web%20Jul%202024%20944.png) and [frame 973](../../screenshots/Slack%20web%20Jul%202024%20973.png), owned by [20-help-community.md](20-help-community.md) and [17-marketing-site.md](17-marketing-site.md), cited only to distinguish the help-centre and marketing-site searches from in-product search; and [frame 801](../../screenshots/Slack%20web%20Jul%202024%20801.png) and [frame 802](../../screenshots/Slack%20web%20Jul%202024%20802.png), owned by [17-marketing-site.md](17-marketing-site.md), cited once in the note immediately below. **Thirty-two** distinct frames are cited in this document in total: these **eleven** plus the twenty-one it owns.
