...
Closure requirement: **every `E-*` cited anywhere in the catalog appears in this table.** **Thirty-five** entities do, and the split is countable off the table itself rather than asserted: **nineteen** are the in-product entities the areas `00`–`16` and `22` own — workspace, user, channel, message, thread, reaction, file, huddle, canvas, list, list record, workflow, app, notification, invitation, external organization, preference, search query and user group — and **sixteen** are public or commercial: the six tenant-scoped billing entities (subscription, billing account, payment method, billing contact, invoice, estimate), the tenant-scoped app subscription owned by [`11-apps-and-integrations.md`](11-apps-and-integrations.md) (`E-APP-SUBSCRIPTION`), the two catalog-scope commercial ones (`E-PLAN`, `E-PROMOTION`), the six content and commerce entities owned by [`17-marketing-site.md`](17-marketing-site.md) (`E-CONTENT-ITEM`, `E-LEAD`, `E-JOB-POSTING`, `E-SERVICE-STATUS`, `E-STORE-PRODUCT`, `E-BASKET`) and the public help query owned by [`20-help-community.md`](20-help-community.md) (`E-HELP-SEARCH-QUERY`). Nineteen plus sixteen is thirty-five, which is the row count of this table and the figure the [closing arithmetic](#omissions) states. The split of the commercial set is a **security boundary rather than a modelling preference**: `E-PLAN` and `E-PROMOTION` are catalog objects that an unauthenticated visitor reads in full on the public pricing page and are owned by [18](18-pricing-plans.md), while a workspace's subscription, billing account, payment method, billing contacts, invoices and estimate each carry an explicit tenant key and are therefore owned by [15](15-admin-workspace.md), the area whose surfaces render them, and a workspace's tier for a third-party app carries the same kind of tenant key and is owned by [11](11-apps-and-integrations.md). Holding both populations in one record would make the read that serves a public page a read of tenant data — which is exactly why an app's published tiers stay on the publicly readable `E-APP` while the workspace's tier for it does not.
...
| `S-SECRET` | Credential, code, session and token handling. Every redeemable artefact is stored **only as a keyed verifier**, never as a raw value at rest, and a credential is not a field of `E-USER` or of any entity a read path returns. |
...
| `S-PAYMENT` | Payment-instrument collection and handling — provider-hosted collection, non-storage beyond the masked field set, redaction, transport and authorization. |
| `S-APPGRANT` | Third-party integration grants — least-privilege scopes, informed grant review, per-install credential isolation, runtime scope enforcement, re-consent on expansion, revocation, audit. |
...
**Entity relationships.** Every relationship below is evidenced by a frame in which both ends are visible together, and **every cardinality is taken from the owning area's contract rather than from what a schema would find convenient**. Three rules govern the reading, because getting them wrong produces a schema that cannot hold what the corpus shows. First, **authorship is a union and it is optional on every branch**: a message may be authored by a person, posted by an app, sent by a workflow, or emitted by the system with no author at all — [`00-product-overview.md`](00-product-overview.md) records the system-message form of `C-MESSAGE-ROW` — so no single author edge may be mandatory. Second, **a public surface has no actor**: the help-centre search and the merchandise basket are reached without a session, so their relationship to `E-USER` is optional at the user end and a build must never require an account to serve them. Third, **commercial facts hang off the tenant record, not the catalogue**: `E-WORKSPACE` relates to `E-SUBSCRIPTION`, which in turn references the `E-PLAN` tier; there is deliberately no direct workspace-to-plan edge, because a plan is a publicly readable catalogue object and an entitlement is a per-workspace resolution of it. The same rule holds a second time for third-party software: a workspace's tier for an **app** is `E-APP-SUBSCRIPTION`, keyed by workspace and app, and the app's own published tiers stay on the publicly readable `E-APP` — so there is no workspace-to-app-tier edge either, and no read that serves a signed-out directory page touches tenant state.
...
erDiagram
    ...
    E-WORKSPACE ||--|| E-SUBSCRIPTION : "has"
    E-WORKSPACE ||--|| E-BILLING-ACCOUNT : "has"
    E-WORKSPACE ||--o{ E-PAYMENT-METHOD : "stores"
    E-WORKSPACE ||--o{ E-BILLING-CONTACT : "designates"
    E-WORKSPACE ||--o{ E-INVOICE : "accrues"
    E-WORKSPACE ||--o| E-ESTIMATE : "is quoted"
    E-SUBSCRIPTION }o--|| E-PLAN : "references tier"
    E-SUBSCRIPTION }o--o| E-PROMOTION : "applies"
    E-WORKSPACE ||--o{ E-APP-SUBSCRIPTION : "subscribes"
    E-APP ||--o{ E-APP-SUBSCRIPTION : "is subscribed to"
    E-PROMOTION }o--|| E-PLAN : "targets tier"
    E-ESTIMATE }o--|| E-PLAN : "prices tier"
    E-CONTENT-ITEM }o--o{ E-CONTENT-ITEM : "relates to"
    E-LEAD }o--o| E-PLAN : "enquires about"
    E-STORE-PRODUCT }o--o{ E-BASKET : "is a line of"
    E-USER |o--o| E-BASKET : "may hold, no account required"
    E-USER |o--o{ E-HELP-SEARCH-QUERY : "may run, no account required"
    E-HELP-SEARCH-QUERY }o--o{ E-CONTENT-ITEM : "returns"
    E-SERVICE-STATUS }o--|| E-SERVICE-STATUS : "rolls up into aggregate"
    E-JOB-POSTING }o--o{ E-CONTENT-ITEM : "is indexed beside"
```
...
- [ ] Every remaining entity — `E-APP`, `E-NOTIFICATION`, `E-FILE`, `E-INVITATION`, `E-EXTERNAL-ORG`, `E-PREFERENCE`, `E-USER-GROUP`, the commercial set `E-PLAN`, `E-PROMOTION`, `E-SUBSCRIPTION`, `E-BILLING-ACCOUNT`, `E-PAYMENT-METHOD`, `E-BILLING-CONTACT`, `E-INVOICE` and `E-ESTIMATE`, the public set `E-CONTENT-ITEM`, `E-LEAD`, `E-JOB-POSTING`, `E-SERVICE-STATUS`, `E-STORE-PRODUCT` and `E-BASKET`, and `E-HELP-SEARCH-QUERY` — is persisted with every field listed for it in the [consolidated data model](#consolidated-data-model), closing the model.
- [ ] The commercial data boundary holds: `E-PLAN` and `E-PROMOTION` carry **no tenant key** and are readable without a session, while every one of the six tenant-scoped commercial entities carries an explicit workspace key and no read of any of them resolves without naming a workspace that `S-AUTHZ-READ` has authorized.
...
| Entities | **35** | every `E-*` cited anywhere appears in the [consolidated data model](#consolidated-data-model) |
