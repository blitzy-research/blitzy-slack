...
Closure requirement: **every `E-*` cited anywhere in the catalog appears in this table.** **Thirty-four** entities do, and the split is countable off the table itself rather than asserted: **nineteen** are the in-product entities the areas `00`–`16` and `22` own — workspace, user, channel, message, thread, reaction, file, huddle, canvas, list, list record, workflow, app, notification, invitation, external organization, preference, search query and user group — and **fifteen** are public or commercial: the six tenant-scoped billing entities (subscription, billing account, payment method, billing contact, invoice, estimate), the two catalog-scope commercial ones (`E-PLAN`, `E-PROMOTION`), the six content and commerce entities owned by [`17-marketing-site.md`](17-marketing-site.md) (`E-CONTENT-ITEM`, `E-LEAD`, `E-JOB-POSTING`, `E-SERVICE-STATUS`, `E-STORE-PRODUCT`, `E-BASKET`) and the public help query owned by [`20-help-community.md`](20-help-community.md) (`E-HELP-SEARCH-QUERY`). Nineteen plus fifteen is thirty-four, which is the row count of this table and the figure the [closing arithmetic](#omissions) states. The split of the commercial set is a **security boundary rather than a modelling preference**: `E-PLAN` and `E-PROMOTION` are catalog objects that an unauthenticated visitor reads in full on the public pricing page and are owned by [18](18-pricing-plans.md), while a workspace's subscription, billing account, payment method, billing contacts, invoices and estimate each carry an explicit tenant key and are therefore owned by [15](15-admin-workspace.md), the area whose surfaces render them. Holding both populations in one record would make the read that serves a public page a read of tenant data.
