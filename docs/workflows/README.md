...
The 99 identifiers below are a **floor, not a ceiling**. An area author who finds a further recurring structure defines it in [`00-product-overview.md`](00-product-overview.md) and adds it here; the closure requirement is that **every `C-*` cited anywhere in the catalog resolves to a definition in that one document and appears in this table**. Seventy of the 98 arrived by exactly that route, in **three waves**. First, [`01-onboarding-and-auth.md`](01-onboarding-and-auth.md) found seven recurring structures with no identifier. Then the in-product areas reported twenty-six more, of which eighteen were genuinely new — contributed by areas 03, 06, 09, 10, 11, 14, 15 and 22 — while the remaining eight proved to be variants of contracts that already existed and were folded into those contracts' own rows rather than given identifiers of their own. Then the four public and commercial areas — 17, 18, 19 and 20 — reported fifty-three structures, of which **forty-five were genuinely new** and eight were folded in as variants or instances. That third wave is why this table nearly doubled: an application shell, its conversations and its console share almost no structure with a marketing property, a pricing comparison, a documentation site or a support centre, and forcing the public structures onto in-product identifiers would have made those identifiers mean two incompatible things. The public contracts each name the in-product contract they are **not** — `C-PUBLIC-NAV` is not `C-TOP-BAR`, `C-NUMBERED-FORM` is not `C-STEP-WIZARD`, `C-COMPARISON-MATRIX` and `C-ORDER-SUMMARY` are not `C-DATA-TABLE`, `C-PUBLIC-TYPEAHEAD` is not `C-TYPEAHEAD-PANEL`, `C-CHOICE-CHIP-GROUP` is not `C-FILTER-CHIP`, `C-CONTENT-CARD` is not `C-TEMPLATE-CARD`, `C-SECTION-NAV` is not `C-TAB-BAR`, `C-DOC-NAV` is not `C-SIDEBAR` — and four of them **replaced** an in-product identifier that an area had been using wrongly, which is recorded as a correction in [`00-product-overview.md`](00-product-overview.md) rather than quietly dropped.
...
| `C-FILTER-CHIP` | Filter chip and chip-based filter bar | [00-product-overview.md](00-product-overview.md) | 00, 02, 07, 08, 09, 11, 16, 17, 20, 21 |
...
| `C-TEMPLATE-CARD` | Template and offer card | [00-product-overview.md](00-product-overview.md) | 00, 10, 11, 17, 20 |
...
| `C-CONTENT-CARD` | Public content card and card grid | [00-product-overview.md](00-product-overview.md) | 00, 20 |
...
| `C-FACET-FILTER-BAR` | Facet filter bar | [00-product-overview.md](00-product-overview.md) | 00 |
| `C-FILTER-MODAL` | Grouped checkbox filter modal | [00-product-overview.md](00-product-overview.md) | 00 |
...
Closure requirement: **every `E-*` cited anywhere in the catalog appears in this table.** Thirty-four entities do — the twenty of the plan's fixed identifier set, and fourteen more that the corpus evidenced during the exhaustive pass — and the split of the commercial set is a **security boundary rather than a modelling preference**: `E-PLAN` and `E-PROMOTION` are catalog objects that an unauthenticated visitor reads in full on the public pricing page and are owned by [18](18-pricing-plans.md), while a workspace's subscription, billing account, payment method, billing contacts, invoices and estimate each carry an explicit tenant key and are therefore owned by [15](15-admin-workspace.md), the area whose surfaces render them. Holding both populations in one record would make the read that serves a public page a read of tenant data.
...
| `E-PAYMENT-METHOD` | [15](15-admin-workspace.md) | **Tenant scope — keyed by workspace.** kind, card or bank account [frame 611](../../screenshots/Slack%20web%20Jul%202024%20611.png) · masked identifier, network, expiry and default flag [frame 674](../../screenshots/Slack%20web%20Jul%202024%20674.png), [frame 679](../../screenshots/Slack%20web%20Jul%202024%20679.png) — **and nothing more**, because `S-SECRET` forbids the instrument itself existing in application storage |
...
erDiagram
    ...
    E-USER ||--o| E-BASKET : "holds"
    E-USER ||--o{ E-HELP-SEARCH-QUERY : "runs publicly"
...
- [ ] Every public form is rate-limited, bot-mitigated and validated server-side under `S-AUTHZ-OP`, and every personal value it collects is handled under `S-PII`; the corpus shows no such control on any capture and that absence is not permission.
...
**One-primary-owner arithmetic, stated plainly.** 443 of the 1,022 frames carry at least one secondary cross-reference in addition to their primary owner, **658 secondary entries in total**, every one of them derived from an area document's own citations rather than hand-authored. Secondary references are deliberately excluded from the coverage arithmetic; if they were counted, the sum would exceed the corpus and prove nothing.
...
| Entities | **34** | every `E-*` cited anywhere appears in the [consolidated data model](#consolidated-data-model) |
| Build-security contracts | **11** | every `S-*` cited anywhere resolves to a contract in [`00-product-overview.md`](00-product-overview.md) |

**One deliverable outside this catalog remains unperformed, and it is deliberate.** The catalog's pages are **not** added to the documentation site's `nav:` in the repository's `mkdocs.yml`. That is the publishing step, it is the only change this work would make outside `docs/`, and it is held until it is explicitly authorised — exactly as [limitation 2](#2-mermaid-diagrams-do-not-render-under-the-committed-site-configuration) holds the superfences fix for the same reason. The consequence for a reader is stated rather than hidden: every catalog page is built by the site and reachable by URL, but none of them appears in the site navigation, so the catalog is browsed from this index or from the repository.
